...
export default function App() {
  ...
  */
  ...
  const retryBatchFile = async (file: BatchUploadFile, event?: React.MouseEvent) => {
  ...
  const handleExportBatchMasterBom = () => {
    ...
    const cell = (value: unknown, type: 'String' | 'Number' = 'String', styleId?: string) => {
      ...
      if (type === 'Number') {
        return `<Cell${style}><Data ss:Type="Number">${numberSafe(value)}</Data></Cell>`;
      }
    ...
    const componentRows = processed.flatMap(({ file, result }) => {
      const estimationResult = result.estimation!;
      const structured = result.structuredBreakdown || estimationResult.structuredBreakdown;
      const parentReference = file.name.replace(/\.[^.]+$/, '');
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
  ...
  <Worksheet ss:Name="Material &amp; Process Rates">
    ...
    link.href = url;
    link.download = `batch-master-bom-${new Date().toISOString().slice(0, 10)}.xls`;
    ...
    <div className="flex flex-col min-h-screen bg-[#f9f9f9] text-[#1a1c1c] font-sans antialiased">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                ...
                >
                  ...
                  <div className="font-mono font-black text-[#004ccd] underline decoration-dotted underline-offset-4">{Number(selectedNestingItem.weightKg || 0).toFixed(3)} kg</div>
                ...
                >
                  ...
                  <div className="font-mono font-black text-[#004ccd] underline decoration-dotted underline-offset-4">{Number(selectedNestingItem.scrapWeightKg || 0).toFixed(3)} kg</div>
                  <div className="mt-1 text-[9px] uppercase font-black text-[#004ccd]">View simple breakdown</div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="font-mono font-black text-[#004ccd] underline decoration-dotted underline-offset-4">{formatInr(selectedNestingItem.scrapValue || 0)}</div>
                  <div className="mt-1 text-[9px] uppercase font-black text-[#004ccd]">View simple breakdown</div>
                </button>
              </div>
            </div>
      ...
      <header className="sticky top-0 z-50 flex justify-between items-center w-full px-6 h-16 bg-white border-b border-[#c3c6d8] shadow-sm">
        ...
        <div className="flex items-center gap-3">
          {currentScreen === 'workspace' && batchUploadFiles.length > 0 && isBatchReady && (
            <button
